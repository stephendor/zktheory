...
export function allContent() {
    ...
    // Create fileToContent map
    const fileToContent = Object.fromEntries(objects.map((e) => [e.__metadata.id, e]));
    
    // IMPORTANT: Create deep copies before reference resolution to preserve original objects
    // This ensures Stackbit's Content Source Interface can still track the original objects
    const resolvedObjects = objects.map(obj => {
        const copy = JSON.parse(JSON.stringify(obj));
        resolveReferences(copy, fileToContent);
        return copy;
    });

    // Use resolved objects for pages URL resolution
    const resolvedPages = resolvedObjects.filter(obj => 
    ...
    const siteConfig = resolvedObjects.find((e) => e.__metadata.modelName === Config.name);

    // Debug: Log all object IDs for Stackbit troubleshooting
    if (process.env.NODE_ENV === 'development') {
        console.log('All content object IDs (with safe reference resolution):');
        resolvedObjects.forEach(obj => {
            console.log(`  - ${obj.__metadata.id} (${obj.__metadata.modelName})`);
        });
        console.log('Site config after resolution:', siteConfig);
    }

    return { objects: resolvedObjects, pages: resolvedPages, props: { site: siteConfig } };
}
