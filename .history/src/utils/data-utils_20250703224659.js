...
export function getAllPosts(objects) {
    console.log('Debug: All objects with PostLayout:', objects.filter(obj => obj.__metadata?.modelName === 'PostLayout').map(obj => ({ 
        filePath: obj.__metadata?.relativeFilePath, 
        title: obj.title 
    })));
    
    return objects.filter((object) => {
        const isPostLayout = object.__metadata?.modelName === 'PostLayout';
        const isInBlogDir = object.__metadata?.relativeFilePath?.includes('pages/blog/');
        const isNotIndex = !object.__metadata?.relativeFilePath?.includes('pages/blog/index.md');
        
        console.log('Debug post:', {
            filePath: object.__metadata?.relativeFilePath,
            title: object.title,
            isPostLayout,
            isInBlogDir,
            isNotIndex,
            included: isPostLayout && isInBlogDir && isNotIndex
        });
        
        return isPostLayout && isInBlogDir && isNotIndex;
    });
}
