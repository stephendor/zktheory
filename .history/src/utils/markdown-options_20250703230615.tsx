...
export const preprocessMarkdown = (content: string): string => {
  ...
  // Replace chart code blocks with Chart components
  content = content.replace(/```chart\n([\s\S]*?)\n```/g, (match, chartData) => {
    console.log('Found chart block:', chartData.substring(0, 100) + '...');
    try {
      // Validate JSON
  ...
  // Replace mermaid code blocks with Mermaid components
  content = content.replace(/```mermaid\n([\s\S]*?)\n```/g, (match, diagramData) => {
    console.log('Found mermaid block:', diagramData.substring(0, 50) + '...');
