...
const MermaidComponent = ({ diagramId, chart, children, ...props }: any) => {
  ...
  // Temporary debug version - just return the data to see if it renders
  return (
    <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
      <h4 className="text-blue-800 text-sm font-bold">Mermaid Diagram Debug</h4>
      <pre className="text-xs text-blue-600 mt-2 overflow-auto whitespace-pre-wrap">{diagramData}</pre>
      <div className="mt-2">
        <Mermaid chart={diagramData} {...props} />
      </div>
    </div>
  );
};
