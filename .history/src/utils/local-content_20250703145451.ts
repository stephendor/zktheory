...
export function allContent() {
    ...
    // Debug logging to see what's in the raw objects before creating fileToContent map
    if (process.env.NODE_ENV === 'development') {
        const headerObj = objects.find(obj => obj.__metadata.id === 'content/data/header.json');
        const footerObj = objects.find(obj => obj.__metadata.id === 'content/data/footer.json');
        console.log('Raw header object before fileToContent mapping:', {
            id: headerObj?.__metadata?.id,
            modelName: headerObj?.__metadata?.modelName,
            hasLogo: !!headerObj?.logo,
            hasPrimaryLinks: !!headerObj?.primaryLinks,
            type: headerObj?.type
        });
        console.log('Raw footer object before fileToContent mapping:', {
            id: footerObj?.__metadata?.id,
            modelName: footerObj?.__metadata?.modelName,
            hasLogo: !!footerObj?.logo,
            hasSocialLinks: !!footerObj?.socialLinks,
            type: footerObj?.type
        });
    }
    ...
    // Debug what's in fileToContent after creation
    if (process.env.NODE_ENV === 'development') {
        const headerInMap = fileToContent['content/data/header.json'];
        const footerInMap = fileToContent['content/data/footer.json'];
        console.log('Header in fileToContent map:', {
            id: headerInMap?.__metadata?.id,
            modelName: headerInMap?.__metadata?.modelName,
            hasLogo: !!headerInMap?.logo,
            hasPrimaryLinks: !!headerInMap?.primaryLinks,
            type: headerInMap?.type
        });
        console.log('Footer in fileToContent map:', {
            id: footerInMap?.__metadata?.id,
            modelName: footerInMap?.__metadata?.modelName,
            hasLogo: !!footerInMap?.logo,
            hasSocialLinks: !!footerInMap?.socialLinks,
            type: footerInMap?.type
        });
    }
